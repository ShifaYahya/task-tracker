...
const [errorMessage, setErrorMessage] = useState("");
  ...
  const handleInput = (e) =>{
    ...
    if(name === 'projectName' && value === "") {
      ...
      setErrorMessage("EnterProject Name o continue")
    }
  ...
  const handleAdd = e  => {
    e.preventDefault();
    if(!projectName) {
      setErrorMessage("Enter Project Name to coninue")
    }else{
  ...
  return (
    ...
    <>
    <button type="button" className='bg-blue-600 uppercase text-white text-md py-1 mx-1.5 pl-2 pr-2.5 font-semibold rounded h-10 w-20  hover:opacity-90 ' onClick={()=> setAddModal(true)}>
     + New
    </button>
    {addModal ? (  //conditional rendering
      <>
      ...
      <form className='px-6 pt-6 pb-4'>
       ...
       <input className='w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4  leading-tight focus:outline-none focus:bg-white' id='project-name'
        name="projectName"
        value={projectName}
      ...
      <div className='flex justify-end p-6 border-t border-slate-200 rounded-b'>
        <button className='bg-blue-600 text-white font-semibold uppercase text-sm px-6 py-3 rounded hover:opacity-90' onClick={handleAdd}>
          Add Task
          </button>
      </div>
      </div>
